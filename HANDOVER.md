...
2. **Pluralization** - The relationship matcher uses simple pluralization (`+ 's'`), so `PolicyId` won't match `policies` (only `policys`). Works correctly for regular plurals like `tenant` → `tenants`.
