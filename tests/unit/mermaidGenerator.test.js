...
describe('mermaidGenerator', () => {
  describe('generateERD', () => {
    ...
    it('should mark FK for relationship properties', () => {
      const schemas = {
        claims: {
          properties: {
            id: { path: 'id', name: 'id', parentPath: null, types: ['guid'], isRequired: true },
            TenantId: { path: 'TenantId', name: 'TenantId', parentPath: null, types: ['guid'], isRequired: true }
          }
        }
      };
      const relationships = [{
    ...
    it('should generate relationship lines', () => {
      ...
      expect(erd).toContain('claims }o--|| tenants');
    });
    ...
    it('should exclude orphan relationships', () => {
      const schemas = { claims: { properties: {} } };
      const relationships = [{
        fromContainer: 'claims',
        fromDatabase: 'db',
  ...
  describe('relationship line formatting', () => {
    it('should extract label from property name', () => {
      ...
      const relationships = [{
        fromContainer: 'claims',
        fromProperty: 'PolicyId',
        toContainer: 'policies',
        cardinality: 'many-to-one',
        isOrphan: false
      }];
      ...
      // Label should be "Policy" (PolicyId with Id removed)
      expect(erd).toContain(': "Policy"');
    });
    ...
    it('should handle nested property paths in label', () => {
      ...
      expect(erd).toContain(': "Details_Policy"');
    });
  });
});
