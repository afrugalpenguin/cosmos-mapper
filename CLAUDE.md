...
- Example values in schema docs: Use synthetic data like `"Example Corp"`, `"POLICY-001"`, `"user@example.com"`
...
- Redact all customer names, policy numbers, claim references
...
### Tenant Databases (one per client/tenant)
Each tenant has their own database containing 6 containers:
- `claims`
- `events`
- `importlogs`
- `movements`
...
All tenant databases share the same schema structure - we only need to sample ONE tenant database to document the template.
...
- Name of one tenant database to use as schema template
...
- Example: `policyId` in claims container suggests relationship to policies container
